...
void TrixTestApp::onDraw(Rectangle rect) {
    ...
    ui::Slot* textSlot = layout.find("text");
    Rectangle titleRect = titleSlot ? titleSlot->rect : rect;
    Rectangle textRect = textSlot ? textSlot->rect : rect;
    drawText("Trix Test App", (int)titleRect.x, (int)titleRect.y, theme.fontSizeTitle, theme.text);
    drawText("Your own debug/benchmark app ;)", (int)textRect.x, (int)textRect.y, theme.fontSize, theme.textDim);
