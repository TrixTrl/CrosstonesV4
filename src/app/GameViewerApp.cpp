...
void GameViewerApp::onStart() {
    ...
    parseAndLoadGame(gamePresets[presetIdx].gameCode);
    currentMove = 0;
}

void GameViewerApp::onFrame() {
...
void GameViewerApp::onDrawOverlay() {
    drawRightPanel();
    ...
    if (gameReplay.size() > 0) {
        ...
        drawText("No moves in this game", panelX(), 56, 16, YELLOW);
    }
    ...
    int ctrlY = 86;
    drawText("A/D: step moves", panelX(), ctrlY, 14, Color{180, 180, 180, 255});
    drawText("BACKSPACE: rewind", panelX(), ctrlY + 18, 14, Color{180, 180, 180, 255});
    drawText("TAB: cycle games", panelX(), ctrlY + 36, 14, Color{180, 180, 180, 255});
