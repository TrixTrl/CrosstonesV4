...
inline void drawBoard(const GamePosition& state, const bool highlights[13][13] = nullptr) {
    ...
    for (int i = 0; i < 13; i++) {
        for (int j = 0; j < 13; j++) {
            ...
            DrawRectangle(i * cellSize, j * cellSize, cellSize, cellSize, col);

            switch (path) {
                case HORIZONTAL:
                    DrawRectangle(i * cellSize, (j + 1.0f/3.0f) * cellSize, cellSize, cellSize / 3, Color{63, 45, 35, 255});
                    break;
                case VERTICAL:
                    DrawRectangle((i + 1.0f/3.0f) * cellSize, j * cellSize, cellSize / 3, cellSize, Color{63, 45, 35, 255});
                    break;
                default: break;
            }
            ...
            if (Piece::tower(piece)) {
                ...
                DrawAppText(hStr.c_str(), cx - textWidth / 2.0f, (j + (1 - textScale) / 2.0f) * cellSize, cellSize * textScale, colors[(centerColorIndex + 1) % 2][0]);
            }
        }
    }
    ...
    if (highlights) {
        for (int i = 0; i < 13; i++) {
            for (int j = 0; j < 13; j++) {
                if (highlights[i][j]) {
                    DrawRectangleLines(i * cellSize, j * cellSize, cellSize, cellSize, YELLOW);
                }
            }
        }
    }
