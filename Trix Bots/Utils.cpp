...
void Utils::print(std::string s, int n, bool newLine)
{
	s += std::to_string(n);
	if (newLine)
		s += "\n";
...
void Utils::print(std::string s, float n, bool newLine)
{
	s += std::to_string(n);
	if (newLine)
		s += "\n";
...
void Utils::print(std::string s, size_t n, bool newLine)
{
	s += std::to_string(n);
	if (newLine)
		s += "\n";
...
float Utils::basicPosEval(bool isWhite, uint8_t (*pieces)[13][13])
{
	...
	if (win == winValue::black)
		return -9999;
	if (win == winValue::white)
		return 9999;
...
float Utils::improvedPosEval(bool isWhite, uint8_t (*pieces)[13][13])
{
	extraBoardData data = generateMetadata(pieces);
	winValue win = gameOver(isWhite, data);
	if (win == winValue::black)
...
int Utils::getBestMoveBasic(bool isWhite, uint8_t (*pieces)[13][13], int depth)
{
	...
	for (int j = 1; j < moves->size(); j++)
	{
		...
		if (gameOver(isWhite, metaData) == (isWhite ? winValue::white : winValue::black))
			return j; // return if we find a forced win for us
...
int Utils::trivialBestMove(bool isWhite, uint8_t (*pieces)[13][13])
{
	...
	for (int j = 1; j < moves->size(); j++)
	{
		...
		if (gameOver(isWhite, &boardCopy) == (isWhite ? winValue::white : winValue::black))
			return j;
...
int Utils::getBestMoveThreaded(bool isWhite, uint8_t (*pieces)[13][13], int depth, int threadCount)
{
	...
	print("getBestMoveThreaded: ", true);
	print("Playing as ");
	print((isWhite ? "White" : "Black"), true);
	...
	// print(debug.sanityCheck, true);
	print("Alpha cutoffs: ", debug.alphaCutoff, true);
	print("Beta cutoffs: ", debug.betaCutoff, true);
...
Utils::extraBoardData Utils::generateMetadata(uint8_t (*pieces)[13][13])
{
	...
	for (int i = 0; i < 13; i++)
	{
		...
		{
			uint8_t piece = (*pieces)[i][j];
			if ((piece & 0b00111111) == 0 || !Piece::isTower(piece))
				continue;
			/*print(i, false);
			print("|", false);
			print(j, false);
...
float Utils::alphaBeta(uint8_t (*pieces)[13][13], int depth, float alpha, float beta, bool maximizing, float (*evalFunc)(bool isWhite, uint8_t (*pieces)[13][13]), debugContainer *debug)
{
	...
	if (depth == 0)
	{ // || (Utils::gameOver(maximizing, pieces) != winValue::none)) {
...
float Utils::alphaBeta_wTable(uint8_t (*pieces)[13][13], int depth, float alpha, float beta, bool maximizing, float (*evalFunc)(bool isWhite, uint8_t (*pieces)[13][13]), uint64_t zobristKey, debugContainer *debug)
{
	if (debug != nullptr)
		debug->n++;
	if (debug != nullptr)
	...
	if (depth == 0)
	{ // || (Utils::gameOver(maximizing, pieces) != winValue::none)) {
		float eval = evalFunc(maximizing, pieces);
	...
	/*
		...
		Figure out when to cache other things into the table and like... what and how
		Make move ordering a thing
	...
	*/
	...
	if (maximizing)
	{
		...
		for (int j = 1; j < moves->size(); j++)
		{
			...
			float alphabeta = alphaBeta_wTable(&boardCopy, depth - 1, alpha, beta, false, evalFunc, ZobristHasher::zobristKey(pieces, &boardCopy, &(*moves)[j], zobristKey), debug);
			value = max(value, alphabeta);
	...
	else
	{
		...
		for (int j = 1; j < moves->size(); j++)
		{
			...
			float alphabeta = alphaBeta_wTable(&boardCopy, depth - 1, alpha, beta, true, evalFunc, ZobristHasher::zobristKey(pieces, &boardCopy, &(*moves)[j], zobristKey), debug);
			value = min(value, alphabeta);
...
void Utils::TrixSearcher::getOrder(std::shared_ptr<std::vector<std::vector<xMove>>> moves, uint8_t (*pieces)[13][13], std::vector<int> *order, int tableValue, bool isWhite, debugContainer *debug)
{
	...
	if (tableValue != -1)
	{
		...
		{
			if (debug != nullptr)
				debug->caughtHashErrors++;
		}
		else
		{
			if (debug != nullptr)
				debug->bestMoveFromTable++;
...
int Utils::TrixSearcher::fullSearch(int depth)
{
	...
	for (int i = 1; i < evaluations.size(); i++)
	{ // skip the null move TO BE CHANGED
		float posEval = evaluations[i];
		if (posEval > (bestEval + newBestThreshold))
		{
			bestMoves.clear();
...
float Utils::TrixSearcher::negamax_wTable(uint8_t (*pieces)[13][13], bool selfPassed, bool opponentPassed, int depth, float alpha, float beta, bool isWhite, uint64_t zobristKey, debugContainer *debug)
{
	...
	if (debug != nullptr && debug->caughtHashErrors > 0)
	{
		...
		if (ZobristHasher::zobristKey(pieces, isWhite) != zobristKey)
			print("shit fuck ");
	}
	...
	if (debug != nullptr)
	{
		if (entry.key == zobristKey && bestMove == entry.best)
			debug->trueBestMoveFromTable++;
	}
