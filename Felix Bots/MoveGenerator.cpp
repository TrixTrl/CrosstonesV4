...
std::unique_ptr<std::vector<dc::Move>> MoveGenerator::getMovesStatic(const bool isWhite, const uint8_t(*pieces)[13][13])
{
	return moveGen.getMoves(isWhite, pieces);
}

std::unique_ptr<std::vector<dc::Move>> MoveGenerator::getMoves(const bool isWhite, const uint8_t(*pieces)[13][13]) {
	std::unique_ptr<std::vector<Move>> moves = std::make_unique<std::vector<Move>>();
	moves->reserve(700);
	...
	for (int i = 0; i < 13; i++) {
		for (int j = 0; j < 13; j++) {
			...
			visited[i][j] = true;
			basicGenerator(*moves, &boardCopy, i, j, i, j, &visited, Piece::maxSteps(piece), false, isWhite, pieces);
		}
	}
...
void MoveGenerator::basicGenerator(std::vector<Move>& moves, uint8_t(*state)[13][13], int x, int y, const int startX, const int startY, bool(*visited)[13][13], int remainingSteps, bool turned, bool isWhite, const uint8_t(*pieces)[13][13])
{
	std::vector<xMove> move = std::vector<xMove>();
	move.reserve(5);
	//save move if start Tower has moved
	if ((*pieces)[startX][startY] != (*state)[startX][startY])
	{
		move.push_back(xMove{ startX, startY, ((uint8_t)(((*pieces)[startX][startY]) ^ ((*state)[startX][startY]))) });
		if (startX != x || startY != y)
			move.push_back(xMove{ x, y, ((uint8_t)(((*pieces)[x][y]) ^ ((*state)[x][y]))) });

		for (int i = 0; i < 13; i++) {
			for (int j = 0; j < 13; j++) {
				if ((*pieces)[i][j] == (*state)[i][j] || (startX == i && startY == j) || (x == i && y == j)) continue;

				move.push_back(xMove{ i, j, ((uint8_t)(((*pieces)[i][j]) ^ ((*state)[i][j]))) });		//Creation of xor lists for moves
			}
		}
	...
	if (!turned && (((*state)[x][y] & hasTurnPiece) != 0)) {		//Turn in place if we can and haven't yet
		...
		basicGenerator(moves, &boardCopy, x, y, startX, startY, &visitedCopy, remainingSteps, true, isWhite, pieces);
	}
	...
	for (int d = 0; d < 4; d++) {		//Loop through the 4 possible directions
		...
		if ((dest & 0b00111111) == 0) {		//Most basic case : empty target square
			for (int splitOff = 1; splitOff <= Piece::height(piece); splitOff++) {		//number of moved pieces
				...
				basicGenerator(moves, &boardCopy, i, j, startX, startY, &visitedCopy, remainingSteps - 1, false, isWhite, pieces);
			}
		}
			...
			for (int splitOff = 1; (splitOff <= Piece::height(piece)) && (splitOff <= 5 - Piece::height(dest)); splitOff++) {
				...
				std::memcpy(&visitedCopy, visited, sizeof(visitedCopy));
				visitedCopy[i][j] = true;
				basicGenerator(moves, &boardCopy, i, j, startX, startY, &visitedCopy, 0, false, isWhite, pieces);
			}
		}
		...
		if (remainingSteps >= 2 && Piece::isTower(dest) && Piece::colour(dest) != Piece::colour(piece)) {		//capturing
			...
			std::memcpy(&visitedCopy, visited, sizeof(visitedCopy));
			visitedCopy[i][j] = true;
			captureGenerator(moves, &boardCopy, x, y, i, j, startX, startY, &visitedCopy, remainingSteps - 1, false, isWhite, pieces);
		}
	}
}


void MoveGenerator::captureGenerator(std::vector<Move>& moves, uint8_t(*state)[13][13], int originX, int originY, int x, int y, const int startX, const int startY, bool(*visited)[13][13], int remainingSteps, bool turned, bool isWhite, const uint8_t(*pieces)[13][13])
{
	/* 
		0
	   3 1
	...
	*/
	...
	if (!turned && (((*state)[x][y] & hasTurnPiece) != 0)) {		//Turn in place if we can and haven't yet
		...
		bool visitedCopy[13][13];
		std::memcpy(&visitedCopy, visited, sizeof(visitedCopy));
		captureGenerator(moves, &boardCopy, originX, originY, x, y, startX, startY, &visitedCopy, remainingSteps, true, isWhite, pieces);
	}
	...
	for (int d = 0; d < 4; d++) {		//Loop through the 4 possible directions
		...
		if (Piece::height(dest) == 0 || (Piece::isTower(dest) && Piece::colour(dest) == Piece::colour(origin)) || Piece::isAddOn(dest)) {
			...
			for (int splitOff = 1; (Piece::height(origin) == 1 ? splitOff <= 1 : (splitOff < Piece::height(origin))) && (splitOff <= 5 - Piece::height(dest)); splitOff++) {
				...
				std::memcpy(&visitedCopy, visited, sizeof(visitedCopy));
				visitedCopy[i][j] = true;
				basicGenerator(moves, &boardCopy, 
					i, j, 
					startX, startY, 
					&visitedCopy, 
					0, //steps
					false, isWhite, pieces); //do last move
			}
		}
			...
			std::memcpy(&visitedCopy, visited, sizeof(visitedCopy));
			visitedCopy[i][j] = true;
			captureGenerator(moves, &boardCopy, 
				originX, originY, 
				i, j, 
				startX, startY, 
				&visitedCopy, 
				remainingSteps - !Piece::isBlue(origin), 
				false, isWhite, pieces);		//only decrement moves if the capturing piece doesn't have a blue addon
		}
	}
}