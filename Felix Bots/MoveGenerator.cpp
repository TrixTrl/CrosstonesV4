...
)
{
	...
	if (!onlyClaimsAndCaptures && origSquares[startX][startY] != currentSquares[startX][startY])
	{
		...
		for (int i = 0; i < 13; i++) {
			for (int j = 0; j < 13; j++) {
				if (origSquares[i][j] == currentSquares[i][j] || (startX == i && startY == j) || (x == i && y == j)) continue;
	...
	for (int d = 0; d < 4; d++) {
		...
		if ((piece & hasTurnPiece) != 0) {
			if ((piece & setTurnPiece) == (d % 2) * setTurnPiece) continue;
		}
		/*   0
		   3 + 1
			 2   */
		...
		if (i < 0 || i > 12 || j < 0 || j > 12) continue; // Bounds
		if (i % 2 + j % 2 == 2) continue;	// Holes
		if (visited[i][j]) continue;		// Revisiting check
		...
		if (Piece::height(dest) == 0 || (Piece::isTower(dest) && Piece::colour(dest) == Piece::colour(origin)) || Piece::isAddOn(dest)) {
			...
			for (int splitOff = 1; (Piece::height(origin) == 1 ? splitOff <= 1 : (splitOff < Piece::height(origin))) && (splitOff <= 5 - Piece::height(dest)); splitOff++) {
				...
				if (Piece::height(origin) == 1) {
					...
					if (Piece::isAddOn(currentSquares[originX][originY])) currentSquares[originX][originY] &= 0b11011111;		//if we left a single addon, remove the color
				}
				...
				//do last move
				basicGenerator(i, j, startX, startY, 0/*steps*/,  false, isWhite, false); 
